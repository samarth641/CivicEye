      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
        {ITEMS.map(({ color, label, shape }) => (
          <div key={label} className="flex items-center gap-1.5">
            <div
              className={`h-2.5 w-2.5 border-2 border-white/90 shadow-sm ${
...
export function Legend({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "map-panel pointer-events-auto absolute bottom-16 left-1/2 z-20 hidden -translate-x-1/2 px-4 py-3 md:block sm:bottom-20 sm:left-[22rem] sm:translate-x-0",
